...
var app = new Vue({
  ...
  methods: {
    isLiked: function isLiked(newsid) {
    ...
    likeNews: function likeNews(id) {
      ...
      axios.post("/api/like-news/" + id).then(function (res) {});
    },
    add_remove_source: function add_remove_source(id) {
    ...
    getNews: function getNews(type) {
      ...
      axios.get(link + "&tid=" + Date.now()).then(function (res) {
        ...
        (_this2$stories = _this2.stories).push.apply(_this2$stories, _toConsumableArray(res.data.data));
      });
    },
    getStory: function getStory() {
      ...
      var news_filter = this.stories.filter(function (s) {
        return s.id == _this3.$route.params.id;
      });
      ...
      axios.get('/api/' + this.lang + '/get-story/' + this.id + "?tid=" + Date.now()).then(function (res) {
        ...
        if (news_filter.length == 0) {
          if (!$(".news-article").hasClass("visible")) _this3.openNews();
        }
  ...
  mounted: function mounted() {
    ...
    $("body").on('click', '.pusher.dimmed', function () {
      if ($(".icon-sidebar").hasClass('visible') && JSON.stringify(_this.selected_sources) != localStorage.getItem("snews_sources_" + _this.lang)) {
        ...
        axios.put('/api/select-sources/' + _this.lang, {
          ...
          if (_this4.$route.name == "home-en") {
            _this.directLoad = false;
            _this.loading = true;
            _this.last_route = {
              name: _this4.$route.name,
              query: _this4.$route.query
            };
            _this.page = 0;
            _this.stories = [];
            _this.source = _this4.category = null;
    ...
    $("body").on('click', '.launch-right.icon.item', function () {
      _this.selected_sources_match = _this4.selected_sources;
